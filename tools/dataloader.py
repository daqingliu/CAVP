...
class DataLoader(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.use_att:
            att_dict = np.load(os.path.join(self.input_att_dir, str(self.info['images'][ix]['id']) + '.npz'))
            att_feat = att_dict['feat']
            fc_feat = np.mean(att_feat, axis=0)
            if self.norm_att_feat:
                att_feat = att_feat / np.linalg.norm(att_feat, 2, 1, keepdims=True)
            if self.use_box:
                box_feat = att_dict['box']
...
class BlobFetcher:
    """Experimental class for prefetch blobs in a separate process."""
