...
class BasicModel(nn.Module):
    ...
    def beam_search(self, init_state, init_logprobs, pre_feats, *args, **kwargs):
        ...
        def beam_step(logprobsf, unaug_logprobsf, beam_size, t, beam_seq, beam_seq_logprobs, beam_logprobs_sum, state, feat):
            # INPUTS:
            ...
            for vix in range(beam_size):
                ...
                new_feat[vix, :] = feat[v['q'], :]  # dimension one is time step
            ...
            state = new_state
            feat = new_feat
            return beam_seq, beam_seq_logprobs, beam_logprobs_sum, state, feat, candidates
        ...
        feat_table = list(pre_feats.chunk(group_size, 0))
        # END INIT
        ...
        for t in range(self.seq_length + group_size - 1):
            for divm in range(group_size):
                if divm <= t <= self.seq_length + divm - 1:
                    ...
                    # the un augmented ones for sorting the candidates in the end. # for historical
                    # reasons :-)
                    unaug_logprobsf = add_diversity(beam_seq_table, logprobsf, t, divm, diversity_lambda, bdash)

                    # infer new beams
                    beam_seq_table[divm], beam_seq_logprobs_table[divm], beam_logprobs_sum_table[divm],\
                    state_table[divm], feat_table[divm],\
                    ...
                    logprobs_table[divm], state_table[divm], output_feat = \
                        self.get_logprobs_state(Variable(it.cuda()), feat_table[divm], *(args[divm] + [state_table[divm]]))
                    feat_table[divm] = torch.cat([feat_table[divm], output_feat.unsqueeze(1)], dim=1)
